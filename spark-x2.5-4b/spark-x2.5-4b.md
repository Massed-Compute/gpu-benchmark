...
Stock `vllm-openai` without the plugin cannot load `Spark2_5ForCausalLM`.
...
Buy A6000 for packed serving and for a chat box billed by the hour. Use Blackwell when 32-way TTFT matters (c32 p99 113 ms vs A6000 342 ms). The c1 p99 gap (21 ms vs 120 ms) is a cold first request over 5 samples, not a tail. Do not treat the c32 medians (98 ms vs 180 ms) as interactive latency.

## Notes
- 4.1B BF16 (~8.3 GB weights) fits 48 GB. Smallest **standard** live SKU that ran it is `gpu_1x_a6000` at $0.57. Same silicon is listed cheaper as `gpu_1x_a6000_low_ram` ($0.55) and `gpu_1x_a6000_spot` ($0.50); those configs were not launched. A30 ($0.35) and A5000 ($0.44) had zero capacity. Multi-GPU is not warranted.
- Same checkpoint on all three rows. Did not bench the advertised 1M context window.
- Serving used `vllm-spark2-5-plugin` inside `vllm/vllm-openai:v0.28.0`. First stock-latest start failed with `Spark2_5ForCausalLM` unsupported.
