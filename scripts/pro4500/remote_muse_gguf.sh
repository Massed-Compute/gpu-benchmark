...
/build/bin/llama-bench -m /models/'"$GGUF"' -ngl 99 -p 128 -n 128 -r 5 -o json | tee /out/llama-bench.json
nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv | tee /out/nvidia-smi.txt
echo DONE > /out/DONE
'
log "DONE $OUTDIR"
cat "$OUTDIR/llama-bench.json" | head -c 2000 || true
